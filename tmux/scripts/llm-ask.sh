#!/usr/bin/env bash
# tmux popup: quick LLM question in current pane directory.
set -euo pipefail
...
main() {
    ...
    if [[ ! -x "$ASK_BIN" ]]; then
        tmux display-message "llm-ask: ask script not found"
        exit 1
    fi

    if ! command -v llm >/dev/null 2>&1; then
        tmux display-message "llm-ask: llm not installed"
        exit 1
    fi
    ...
    case "$MODE" in
        ...
        pane)
            ...
            [[ -n "$QUESTION" ]] || {
                tmux display-message "llm-ask: empty question"
                exit 1
            }
            if [[ -n "$TEMPLATE" ]]; then
