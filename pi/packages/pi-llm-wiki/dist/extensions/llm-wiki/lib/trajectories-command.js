...
export function registerWikiTrajectoriesCommand(pi) {
    pi.registerCommand("wiki-trajectories", {
        description: "Enable or disable agent-trajectory working-memory (capture/distill/recall). Off by default.",
        handler: async (args, ctx) => {
            const arg = args.trim().toLowerCase();
            const current = trajectoriesEnabled(loadTaskConfig(ctx.cwd));
            if (!arg) {
                ctx.ui.notify(`LLM Wiki trajectories are ${current ? "ON" : "OFF"}. Use \`/wiki-trajectories on\` or \`off\`.`, "info");
                return;
            }
            let next;
            if (ON_WORDS.has(arg))
                next = true;
            else if (OFF_WORDS.has(arg))
                next = false;
            else {
                ctx.ui.notify(`LLM Wiki: could not parse "${args.trim()}". Use \`on\` or \`off\`.`, "error");
                return;
            }
            if (next === current) {
            ...
            ctx.ui.notify(`LLM Wiki: trajectory tools ${next ? "enabled" : "disabled"} — reloading extensions…`, "info");
