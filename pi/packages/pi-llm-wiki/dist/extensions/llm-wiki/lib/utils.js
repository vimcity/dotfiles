 ...
 */
export function detectVaultFormat(dir) {
 ...
 */
export function getPersonalWikiRoot() {
...
export function getPersonalWikiPaths() {
 ...
 */
export function migrateDoubledPersonalVault(parentRoot = getPersonalWikiRoot()) {
...
export async function exec(pi, command, args, options) {
    const result = await pi.exec(command, args, options ?? {});
    if (result.killed)
        throw new Error(`Command timed out or was aborted: ${command}`);
    if (result.code !== 0) {
        const detail = result.stderr.trim();
        throw new Error(`Command failed (${command} exited ${result.code})${detail ? `: ${detail}` : ""}`);
    }
    return result;
}
...
function realpathWithMissingTail(path, seen = new Set()) {
    let current = path;
    const tail = [];
    while (true) {
        try {
            ...
            if (stat.isSymbolicLink()) {
                ...
                if (seen.has(symlinkPath))
                    throw new Error(`Cannot resolve symlink cycle: ${path}`);
...
export function relativePhysicalPath(rootPath, candidatePath) {
...
export function isProtectedPath(absPath, paths) {
    try {
