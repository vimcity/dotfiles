...
def analyse(filepath):
    ...
    print()
    print(f'{"Turn":<8} {"Gap":<12} {"cacheRead":>10} {"input":>8} {"output":>6} {"status":<12}')
    print('-'*65)
    ...
    for l in lines:
        if l.get('type') == 'message' and l['message'].get('role') == 'assistant':
            ...
            marker = ' 🧊' if is_miss else '  '
            print(f'{msg_id:<8} {gap:<12} {read:>10} {inp:>8} {out:>6} {reason:<12}{marker}')

    if misses:
        print()
        n = len(misses)
        print(f'🚨 {n} cache miss(es) detected')
        for m in misses:
            print(f'  Turn {m[0]} (gap={m[1]}) — cacheRead={m[2]}, input={m[3]}')
        print()
        print('Causes:')
        print('  1. Extension before_agent_start hook → check if it modifies systemPrompt')
        print('  2. Extension injects custom messages → changes messages prefix, busts cache')
